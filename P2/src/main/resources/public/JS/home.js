async function retrieveData() {
    ...
    if (checkingResponse.status === 200) {
        ...
        document.getElementById("accountBalance").innerText =
            formatter.format(checkingAccount.balance);
    ...
    if(savingsResponse.status===200){
        ...
        document.getElementById("savingsBalance").innerText = formatter.format(savingsAccount.savingsBalance);
        document.getElementById("savingsNumber").innerText = savingsAccount.savingsAccountNumber;
    ...
    if(loanAppResponse.status===200){
        ...
        console.log(loanApp);
        loanApp.forEach(app =>{updateTable(app)
...
function updateTable(loanApp){
    ...
    newAppRow.append(loanAppIdTD, loanStatusTD, loanAppAmountTD, termInMonthsTD);
